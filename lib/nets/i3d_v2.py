...
class i3d(Network):
    ...
    def _head_to_tail(self, pool5, is_training, reuse=None):
        with tf.variable_scope(self._scope, self._scope, reuse=reuse):
            ...
            self.print_tensor_infomation(net,'mixed_5c')
            ...
            with tf.variable_scope(end_point):
                ...
                net = tf.nn.avg_pool3d(net, ksize=[1, 2, 3, 3, 1],
                                       strides=[1, 1, 1, 1, 1], padding=snt.VALID)

                net = tf.nn.dropout(net, 1.0)
                logits = Unit3D(output_channels=4096,
                                kernel_shape=[1, 1, 1],
                                activation_fn=None,
                                use_batch_norm=False,
                                use_bias=True,
                                name='Conv3d_0c_1x1')(net, is_training=is_training)
                # end_points['logits_initial'] = logits

                # if self._spatial_squeeze:
                logits = tf.squeeze(logits, [2, 3], name='SpatialSqueeze')
                logits = tf.reduce_mean(logits, axis=1)

            pool5_flat = slim.flatten(logits, scope='flatten')
